...
from ioet_feature_flag.providers import YamlGitRemoteProvider, Provider
...
class TestYamlGitRemoteProvider:
    def test_get_toggle_list(
        ...
        mock_request(yaml.dump(toggles))
        toggle_provider: Provider = YamlGitRemoteProvider(
            project_id="test",
            environment="test",
        )
    ...
    def test_load_toggles_from_file_raises_error_when_file_format_invalid(
        ...
        with pytest.raises(InvalidToggleFileFormat) as error:
            provider = YamlGitRemoteProvider(
                project_id="test",
                environment="test",
            )
            provider.get_toggle_list()

        expected_message = "The provided file doesn't have a valid YAML format."
        assert str(error.value) == expected_message
    ...
    def test_load_toggles_request_called_with_right_parameters(
        ...
        patched_request = mock_request(yaml.dump(toggles))
        toggle_provider: Provider = YamlGitRemoteProvider(
        ...
        patched_request.assert_called_with(
            method="GET",
            url="https://test-remote.com/test-user/test-repo/main/test-project/test-env.yaml",
            headers=None,
        )
        assert toggle_list == ["another_toggle", "some_toggle"]
